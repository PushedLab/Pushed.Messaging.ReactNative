...
private var gOriginalAppDelegateClass: AnyClass?
...
public class PushedIosLib: NSProxy {
  ...
  ///Init librarry
    public static func setup(_ appDel: UIApplicationDelegate) -> String {
      addLog("Start setup")
      pushedToken=nil
  ...
  public static func stop(_ appDel: UIApplicationDelegate) {
    addLog("Stop pushed")
    guard let appDelegate = UIApplication.shared.delegate,
        let originalClass = gOriginalAppDelegateClass else {
      addLog("Cannot unproxy AppDelegate. Either AppDelegate is nil or the original class was not stored.")
      return
    }

    if object_setClass(appDelegate, originalClass) != nil {
      addLog("Successfully restored the original AppDelegate class.")
    } else {
      addLog("Failed to restore the original AppDelegate class.")
    }
  }
  ...
  private static func proxyAppDelegate(_ appDelegate: UIApplicationDelegate?) {
      ...
      gOriginalAppDelegateClass = type(of: appDelegate)
